...
public class EZLerp : MonoBehaviour
{
    ...
    */
    ...
    /// <param name="delay">Delay before function start</param>
    /// <param name="callback">Action call at the end of function</param>
    public void Lerp(Easing.Type easingType, UnityEngine.UI.Image target, Color start, Color end, float duration, float delay, Action callback)
    {
        StartCoroutine(COLerp(easingType, target, start, end, duration, delay, callback));
    }
    ...
    //------------------------------------------------------------------------------------------------------------
    IEnumerator COLerp(Easing.Type easingType, UnityEngine.UI.Image target, Color start, Color end, float duration, float delay, Action callback)
    {
        float currentLerpTime = 0;
        float perc = 0;
        float progress = 0;
